...
# ### CNN for MJO Compute STRONG Bivariate Correlation - V1
# ### Ben Crair
# ### March 21st
...
for i in range(num_ensembles):
    ...
    for j in range(num_rounds):
        in1 = Dataset(directory + "CNNMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(i+1)+"_rund"+str(j+1)+"_Prdct_Train.nc")
        in2 = Dataset(directory + "CNNMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(i+1)+"_rund"+str(j+1)+"_Prdct_Val.nc")
        in3 = Dataset(directory + "CNNMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(i+1)+"_rund"+str(j+1)+"_Prdct_Test.nc")
    ...
    if sum_train_preds is None:
        sum_train_preds   = train_preds.copy()
        sum_train_targets = train_targets.copy()
        sum_val_preds     = val_preds.copy()
        sum_val_targets   = val_targets.copy()
        sum_test_preds    = test_preds.copy()
        sum_test_targets  = test_targets.copy()
    else:
...
filename = directory + "STRONG_BCOR_by_leadTm.csv"
