...
# take main training script and copy for each ensemble and lead time
...
do
  ...
  do
    ...
    cp ViT_MJO_Ben_MDL_Train.py temp.py
    `sed -i "s/seed_num = 1/seed_num = $j/" temp.py`
    `sed -i "s/leadTm = 15/leadTm = $i/" temp.py`
    `mv temp.py ViT_MJO_Ben_MDL_Train_leadTm${i}_ensm${j}.py`
  done
done
