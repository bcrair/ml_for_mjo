# ## Compute Predictions for Strong MJO Events 
# #### Ben Crair
# #### May 20th, 2025
...
in6  = Dataset(directory + "CML2025_Step0C_OBS_remapped_90x180_daily_DJFM_nonFltr_PC1_leadTm"+str(leadTm)+".nc") # Current RMM1 Value
in7  = Dataset(directory + "CML2025_Step0C_OBS_remapped_90x180_daily_DJFM_nonFltr_PC2_leadTm"+str(leadTm)+".nc") # Current RMM2 Value
...
train_current_index_1 = np.delete(current_index,   np.s_[j*pct:(j+1)*pct], axis=0)
...
test_current_index_1  = current_index[j*pct:(j+1)*pct]
test_y_1  = target[j*pct:(j+1)*pct]

test_t_1  = np.shape(test_y_1)[0] # time-dim of the test dataset

train_data_1 = OBSDataset(train_x_1, train_current_index_1, train_y_1)
...
test_dataset_1 = OBSDataset(test_x_1, test_current_index_1, test_y_1)
...
train_current_index_2 = np.delete(current_index,   np.s_[j*pct:], axis=0)
train_y_2 = np.delete(target, np.s_[j*pct:], axis=0)

# test datasets (formulate the test dataset by taking the cutoff[j*pct:(j+1)*pct] when formulating the training dataset)
test_x_2  = data  [j*pct:]
test_current_index_2 = current_index[j*pct:]
test_y_2  = target[j*pct:]

test_t_2  = np.shape(test_y_2)[0] # time-dim of the test dataset

train_data_2 = OBSDataset(train_x_2, train_current_index_2, train_y_2)
...
test_dataset_2 = OBSDataset(test_x_2, test_current_index_2, test_y_2)
...
train_current_index_3 = np.delete(current_index,   np.s_[j*pct:(j+1)*pct], axis=0)
...
test_current_index_3 = current_index  [j*pct:(j+1)*pct]
test_y_3  = target[j*pct:(j+1)*pct]

test_t_3  = np.shape(test_y_3)[0] # time-dim of the test dataset

train_data_3 = OBSDataset(train_x_3, train_current_index_3, train_y_3)
...
test_dataset_3 = OBSDataset(test_x_3, test_current_index_3, test_y_3)
...
# ### Section D: Compute Final Predictions

dirSave = "/home/bec32/project/ML_for_MJO/ViT/Ensemble_Results/Result_03/"
for index, round in enumerate(train_val_test_rounds):
    ...
    #load model
    model = torch.load("/home/bec32/project/ML_for_MJO/ViT/Ensemble_Results/Result_02/ViTMJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_round"+str(index + 1)+'.pth', weights_only=False).to(device)
    model.eval()
    all_train_preds = []
    all_train_targets = []
    all_val_preds = []
    ...
    with torch.no_grad():
        for batch_data, batch_current_index, batch_target in train_dataloader:
            ...
            mjo_index = torch.sqrt(batch_current_index[:, 0] ** 2 + batch_current_index[:, 1] ** 2)
            mask = mjo_index >= 1.0

            masked_batch_data = batch_data[mask]
            masked_batch_current_index = batch_current_index[mask]
            masked_batch_target = batch_target[mask]
            
            y = model(masked_batch_data)
           
            all_train_preds.append(y.detach().cpu())
            all_train_targets.append(masked_batch_target.detach().cpu())

        for batch_data, batch_current_index, batch_target in val_dataloader:
            ...
            all_val_targets.append(masked_batch_target.detach().cpu())
        ...
        for batch_data, batch_current_index, batch_target in test_dataloader:
            ...
            pred_y = model(masked_batch_data)

            all_test_preds.append(pred_y.detach().cpu())
            all_test_targets.append(masked_batch_target.detach().cpu())
    ...
    os.system("rm -rf " + dirSave + "ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc")
    create_NC = Dataset(dirSave+"ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc", "w", format="NETCDF4")
    create_NC.createDimension("time", len(all_train_targets))
    ...
    os.system("rm -rf " + dirSave + "ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc")
    create_NC = Dataset(dirSave+"ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc", "w", format="NETCDF4")
    create_NC.createDimension("time", len(all_val_targets))
    create_NC.createDimension("var",  2)
    #create_NC.createDimension("var3", 3) # (ASXM)
    ...
    os.system("rm -rf " + dirSave + "ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Test.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Test.nc")
    create_NC = Dataset(dirSave+"ViTMJO_STRONG_CORR_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Test.nc", "w", format="NETCDF4")
    create_NC.createDimension("time", len(all_test_targets))
    create_NC.createDimension("var",  2)
    #create_NC.createDimension("var3", 3) # (ASXM)
