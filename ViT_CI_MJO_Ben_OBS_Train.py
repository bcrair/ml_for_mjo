...
for index, round in enumerate(train_val_rounds):
    ...
    #load model
    model  = torch.load("/home/bec32/project/ML_for_MJO/ViT_CI/Ensemble_Results/Result_01/ViT_CI_MJO_Ben_MDL_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+".pth", weights_only=False).to(device)

    model.vit.pos_embedding.requires_grad = False

    for param in model.vit.to_patch_embedding.parameters():
        param.requires_grad = False

    for param in model.vit.transformer.norm.parameters():
        param.requires_grad = False
    
    for block in model.vit.transformer.layers[:]:
        for param in block.parameters():
            param.requires_grad = False

    for param in model.combined_head.parameters():
        param.requires_grad = True

    for param in model.vit.mlp_head.parameters():
        param.requires_grad = False

    model.vit.cls_token.requires_grad = False
        
    
    batches_per_dataset = len(train_dataloader)
    print(f"batches per dataset: {batches_per_dataset}")
    
    # Optimizer, note that we use weight decay with coefficinet 1e-2
...
dirSave = "/home/bec32/project/ML_for_MJO/ViT_CI/Ensemble_Results/Result_03/"
for index, round in enumerate(train_val_test_rounds):
    ...
    #load model
    model = torch.load("/home/bec32/project/ML_for_MJO/ViT_CI/Ensemble_Results/Result_02/ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_round"+str(index + 1)+'.pth', weights_only=False).to(device)
    model.eval()
    all_train_preds = []
    all_train_targets = []
    all_val_preds = []
    ...
    os.system("rm -rf " + dirSave + "ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc")
    ...
    os.system("rm -rf " + dirSave + "ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc")
    ...
    os.system("rm -rf " + dirSave + "ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Test.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViT_CI_MJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Test.nc")
