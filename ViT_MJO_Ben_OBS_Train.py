...
for index, round in enumerate(train_val_rounds):
    ...
    #load model
    model  = torch.load("/home/bec32/project/ML_for_MJO/ViT/Ensemble_Results/Result_01/ViTMJO_Ben_MDL_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+".pth", weights_only=False).to(device)

    #feeze all parameters of model expect for MLP head
...
for index, round in enumerate(train_val_test_rounds):
    ...
    os.system("rm -rf " + dirSave + "ViTMJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViTMJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Train.nc")
    ...
    os.system("rm -rf " + dirSave + "ViTMJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc")
    # wrt to netCDF
    os.system("rm -rf " + dirSave + "ViTMJO_Ben_OBS_leadTm"+str(leadTm)+"_ensm"+str(seed_num)+"_rund"+str(index+1)+"_Prdct_Val.nc")
